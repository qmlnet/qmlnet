...
Q_DECL_EXPORT int qqmlapplicationengine_registerType(NetTypeInfoContainer* typeContainer, int aotTypeId, LPWSTR uri, int versionMajor, int versionMinor, LPWSTR qmlName) {
    ...
    if(typeContainer != nullptr) {
        ...
        AotTypeInfo* aotTypeInfo = NetTypeManager::getAotInfo(aotTypeId);
        if(aotTypeInfo == nullptr) {
            qFatal("Unknown aot type id: %d", aotTypeId);
        }
        return aotTypeInfo->registerQmlFunction(uriString.toUtf8().data(), versionMajor, versionMinor, qmlNameString.toUtf8().data());
    }
}
...
Q_DECL_EXPORT int qqmlapplicationengine_registerSingletonTypeNet(NetTypeInfoContainer* typeContainer, int aotTypeId, LPWCSTR uri, int versionMajor, int versionMinor, LPWCSTR typeName)
{
    ...
    if(typeContainer != nullptr) {
        ...
        return aotTypeInfo->registerQmlSingletonFunction(uriString.toUtf8().data(), versionMajor, versionMinor, typeNameString.toUtf8().data());
    }
}
